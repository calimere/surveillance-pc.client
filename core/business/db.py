...
class Process(BaseModel):
    ...
    prc_is_running = BooleanField(default=False)
...
class ProcessInstance(BaseModel):
    ...
    pri_score = IntegerField(
        default=0
    )  # 100 = très suspect, 0 = pas de suspicion particulière
    pri_score_computed = BooleanField(
        default=False
    )  # indique si le score a déjà été calculé pour cette instance ou s'il doit être recalculé (ex: après une mise à jour du processus en "watched" ou "dangerous")

    pri_populated = BooleanField(
        default=False
    )  # indique si les données de cette instance ont déjà été utilisées pour le calcul du score de ses enfants (pour éviter de prendre en compte plusieurs fois la même instance dans le calcul du score des enfants)
...
def init_db():
    ...
    logger.debug(
        f"Vérification de l'existence de la base de données à {get_db_path()}..."
    )
...
def get_known_watched_processes():
    db.init(get_db_path())
    return Process.select().where(
        (Process.prc_is_watched == True) & (Process.prc_is_unknown == False)
    )
...
def get_known_blocked_processes():
    db.init(get_db_path())
    return Process.select().where(
        (Process.prc_is_dangerous == True) & (Process.prc_is_unknown == False)
    )
...
def update_launched_status(exe_id, launched):
    db.init(get_db_path())
    if (
        Process.update(prc_is_running=launched)
        .where(Process.prc_id == exe_id)
        .execute()
        == 1
    ):
    ...
    for attempt in range(max_retries):
        try:
            event = ProcessEvent.create(
                pri_id=pri_id,
                pev_type=pev_type.value
                if hasattr(pev_type, "value")
                else str(pev_type),
...
def add_events_batch(events_data):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.warning(
                    f"Database locked during batch insert, retry {attempt + 1}/{max_retries}"
                )
                import time
                ...
                logger.error(
                    f"Failed to batch insert events after {max_retries} attempts: {e}"
                )
                raise
...
def set_process_watched_dangerous(prc_id):
    db.init(get_db_path())
    Process.update(prc_is_watched=True, prc_is_dangerous=True).where(
        Process.prc_id == prc_id
    ).execute()
...
def get_non_populate_process_instance():
    db.init(get_db_path())
    return list(ProcessInstance.select().where(ProcessInstance.pri_populated == False))


def set_process_instance_populated(
    instance_id, signed_by, signed_thumbprint, signed_is_ev, owner
):
    db.init(get_db_path())
    result = (
        ProcessInstance.update(
            ...
            pri_owner=owner,
            pri_populated=True,
            pri_signed=True if signed_by else False,
        )
        .where(ProcessInstance.pri_id == instance_id)
        .execute()
    )
    return result == 1


def add_process_instance(
    prc_id,
    pri_timestamp,
    pri_pid,
    pri_ppid,
...
def get_not_compute_process_instance():
    db.init(get_db_path())
    return list(
        ProcessInstance.select().where(ProcessInstance.pri_score_computed == False)
    )


def update_process_instance_score(pri_id, score):
    db.init(get_db_path())
    ProcessInstance.update(pri_score=score, pri_score_computed=True).where(
        ProcessInstance.pri_id == pri_id
    ).execute()
...
def get_process_instance_by_pid(pid, start_time):
    ...
    try:
        return ProcessInstance.get(
            (ProcessInstance.pri_pid == pid)
            & (ProcessInstance.pri_start_time == start_time)
        )
    except DoesNotExist:
        return None
...
def stop_process_instance(pri_id):
    db.init(get_db_path())
    ProcessInstance.update(pri_is_running=False).where(
        ProcessInstance.pri_id == pri_id
    ).execute()
...
def get_running_instances_without_score():
    db.init(get_db_path())
    return list(
        ProcessInstance.select().where(
            (ProcessInstance.pri_is_running == True)
            & (ProcessInstance.pri_score_computed == False)
        )
    )
...
def get_running_instances():
    db.init(get_db_path())
    return list(
        ProcessInstance.select().where((ProcessInstance.pri_is_running == True))
    )
...
def get_recent_process_instances(hours=24):
    ...
    return list(
        ProcessInstance.select().where(ProcessInstance.pri_timestamp >= cutoff_time)
    )
...
def add_or_update_config(cfg_key, cfg_value):
    ...
    try:
        ...
        config = Config.create(cfg_key=cfg_key, cfg_value=cfg_value, created=now)
        cfg_id = config.cfg_id
