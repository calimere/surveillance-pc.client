...
class SyncWorker(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                if total_synced > 0:
                    logger.info(
                        f"✅ {total_synced} enregistrements synchronisés en {duration:.2f}s"
                    )
    ...
    def _sync_processes(self) -> int:
        ...
        try:
            ...
            unsync_processes = list(
                Process.select()
                .where((Process.sync_status == 0) | (Process.sync_status == 2))
                .limit(50)
            )  # Limiter le batch pour éviter surcharge
    ...
    def _sync_process_instances(self) -> int:
        ...
        try:
            unsync_instances = list(
                ProcessInstance.select()
                .where(
                    (ProcessInstance.sync_status == 0)
                    | (ProcessInstance.sync_status == 2)
                )
                .limit(50)
            )
    ...
    def _sync_process_events(self) -> int:
        ...
        try:
            unsync_events = list(
                ...
                .limit(100)
            )  # Plus d'événements car ils sont plus petits
    ...
    def _sync_single_process(self, process: Process) -> bool:
        """Synchronise un processus individuel via HTTP"""
        try:
            return add_processes([process])
        except Exception as e:
            logger.debug(f"Échec sync processus {process.prc_id}: {e}")
            return False

    def _sync_single_instance(self, instance: ProcessInstance) -> bool:
        """Synchronise une instance individuelle via HTTP"""
        try:
            return add_process_instances([instance])
        except Exception as e:
            logger.debug(f"Échec sync instance {instance.pri_id}: {e}")
            return False

    def _sync_single_event(self, event: ProcessEvent) -> bool:
        """Synchronise un événement individuel via HTTP"""
        try:
            return add_process_events([event])
        except Exception as e:
    ...
    def _add_backoff(self, key: str):
        ...
        if key not in self.error_backoff:
            self.error_backoff[key] = {
                "attempts": 1,
                "retry_after": datetime.datetime.now() + datetime.timedelta(minutes=1),
            }
        else:
            ...
            self.error_backoff[key] = {
                ...
                + datetime.timedelta(minutes=backoff_minutes),
            }
    ...
    def _count_unsync_records(self) -> Dict[str, int]:
        ...
        try:
            return {
                ...
                .where((Process.sync_status == 0) | (Process.sync_status == 2))
                .count(),
                "instances": ProcessInstance.select()
                .where(
                    (ProcessInstance.sync_status == 0)
                    | (ProcessInstance.sync_status == 2)
                )
                ...
                .count(),
                "queue": Queue.select()
                .where((Queue.sync_status == 0) | (Queue.sync_status == 2))
                .count(),
            }
        except Exception as e:
...
def mark_for_resync(table_name: str, record_id: int = None):
    ...
    try:
        ...
        logger.info(
            f"✅ {count} enregistrements marqués pour re-synchronisation dans {table_name}"
        )
        return True
...
def reset_sync_errors():
    ...
    try:
        ...
        for table in tables:
            count = (
                table.update(sync_status=0, sync_timestamp=None)
                .where(table.sync_status == 2)
                .execute()
            )

            if count > 0:
                logger.info(
                    f"🔄 {count} enregistrements en erreur reset dans {table._meta.table_name}"
                )
                total_reset += count

        if total_reset > 0:
            logger.info(
                f"✅ {total_reset} enregistrements total remis en attente de synchronisation"
            )
